...
class DayLowBreakdownRule:
    name = EventType.DAY_LOW_BREAKDOWN.value
    ...
    def evaluate(
        ...
        *,
        ...
        del previous
        ref = current.session_low_ref
        if ref is None or ref <= 0:
            return []
        obs = current.session_low_obs
        if obs >= ref:
            return []
        excess = (ref - obs) / ref
        return [
            build_event(
                current=current,
                event_type=EventType.DAY_LOW_BREAKDOWN,
